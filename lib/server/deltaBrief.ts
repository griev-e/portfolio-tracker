...
import type {
  ...
  DeltaBriefRequest,
  DeltaBriefResponse,
} from "@/lib/delta/intelligence";
import { usdCost } from "@/lib/server/cost";

/**
 * delta's money brief: one Claude call per day per ledger shape, cached in
...
const cache = new Map<string, { at: number; data: DeltaBriefResponse }>();
...
export function deltaBriefRateLimited(): boolean {
  const now = Date.now();
...
export function deltaBriefConfigured(): boolean {
  return !!process.env.ANTHROPIC_API_KEY;
}

export function deltaBriefErrorResponse(err: unknown): { status: number; error: string } {
...
export function deltaBriefFingerprint(req: DeltaBriefRequest): string {
...
export function getCachedDeltaBrief(key: string): DeltaBriefResponse | null {
  const hit = cache.get(key);
  if (hit && Date.now() - hit.at < TTL) return hit.data;
  return null;
}

export function setCachedDeltaBrief(key: string, data: DeltaBriefResponse): void {
...
const SYSTEM = `You are the money-brief writer for delta, a private personal-finance terminal. You receive a JSON snapshot of one person's finances this month: net worth and its change, income, spending, savings rate, spending by category, budgets (limit vs spent), savings goals, and upcoming recurring charges.
...
function buildUserMessage(req: DeltaBriefRequest): string {
  return [
...
export async function generateDeltaBrief(
  req: DeltaBriefRequest
): Promise<{ brief: DeltaBrief; costUSD: number | null }> {
  ...
  return {
    brief: JSON.parse(text.text) as DeltaBrief,
