...
import { AddBudgetModal } from "@/components/delta/modals";
import { ActionButton, DeltaEmpty, IconButton, PlusIcon, TrashIcon } from "@/components/delta/ui";
import { Card, CardHeader } from "@/components/ui/Card";
import { PageHeader } from "@/components/ui/PageHeader";
import { Stat } from "@/components/ui/Stat";
import { CATEGORY_COLOR } from "@/lib/delta/data";
import { ledgerHasData, useDelta } from "@/lib/delta/store";
import { fmtPct, fmtUSD } from "@/lib/format";

export default function BudgetsPage() {
  const { ready, ledger, view, setBudgetLimit, removeBudget } = useDelta();
  const [adding, setAdding] = useState(false);

  if (!ready) return null;
  if (!ledger || !view || !ledgerHasData(ledger)) return <DeltaEmpty page="Budgets" />;
