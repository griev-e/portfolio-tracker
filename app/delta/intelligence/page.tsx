...
import type { DeltaBrief, DeltaSnapshot } from "@/lib/delta/intelligence";
import { ledgerHasData, useDelta } from "@/lib/delta/store";
import { fmtPct, fmtUSD } from "@/lib/format";

type State =
  | { kind: "loading" }
  | { kind: "ready"; brief: DeltaBrief; cached: boolean; costUSD: number | null }
  | { kind: "offline" } // no API key
  | { kind: "error"; message: string };

export default function DeltaIntelligencePage() {
  const { ready, ledger, view } = useDelta();
  const [state, setState] = useState<State>({ kind: "loading" });
  const reqId = useRef(0);

  const buildSnapshot = useCallback((): DeltaSnapshot | null => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/delta-brief", {
        method: "POST",
  ...
  if (!ledger || !view || !ledgerHasData(ledger)) return <DeltaEmpty page="Intelligence" />;
      ...
      {state.kind === "offline" && (
        <Card className="px-6 py-8 text-center" i={4}>
          ...
          <p className="mx-auto mt-2 max-w-md text-[13px] leading-relaxed text-mute">
            ...
            Claude-written monthly money briefs. Everything else in delta works without it — your
...
function BriefView({ brief, cached, costUSD }: { brief: DeltaBrief; cached: boolean; costUSD: number | null }) {
  return (
