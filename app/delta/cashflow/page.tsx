...
import { MoneyFlowBars } from "@/components/delta/bits";
import { Card, CardHeader } from "@/components/ui/Card";
import { PageHeader } from "@/components/ui/PageHeader";
import { Stat } from "@/components/ui/Stat";
import { CASH_FLOW } from "@/lib/delta/data";
import { fmtPct, fmtUSD } from "@/lib/format";

export default function CashFlowPage() {
  const months = CASH_FLOW.length;
  const avgIncome = CASH_FLOW.reduce((s, m) => s + m.income, 0) / months;
  const avgExpense = CASH_FLOW.reduce((s, m) => s + m.expenses, 0) / months;
    ...
    <div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Card className="px-5 py-4" i={2} hover={false}>
          <Stat label="Avg net" value={avgNet} format={(v) => `+${fmtUSD(v, true)}`} size="sm" />
        </Card>
      ...
      <Card className="mb-5 px-5 py-6" i={4}>
        <CardHeader eyebrow="Trailing 6 months" title="Income vs. spending" className="mb-6" />
        <MoneyFlowBars data={CASH_FLOW} height={220} />
      </Card>
      ...
      <Card className="overflow-hidden" i={5}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] text-[13px]">
            ...
            <tbody>
              {[...CASH_FLOW].reverse().map((m) => {
                const net = m.income - m.expenses;
                const rate = m.income > 0 ? net / m.income : 0;
                return (
                  <tr key={m.month} className="border-b border-edge/60 last:border-0 hover:bg-white/[0.03]">
                    <td className="px-6 py-3 text-ink">{m.month} 2026</td>
                    ...
                    <td className="px-6 py-3 text-right font-mono tnum text-ink">+{fmtUSD(net, true)}</td>
