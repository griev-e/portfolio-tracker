...
import type { DeltaBriefRequest, DeltaSnapshot } from "@/lib/delta/intelligence";
import {
  deltaBriefConfigured,
  deltaBriefErrorResponse,
  deltaBriefFingerprint,
  deltaBriefRateLimited,
  generateDeltaBrief,
  getCachedDeltaBrief,
  setCachedDeltaBrief,
} from "@/lib/server/deltaBrief";
...
function parseBody(body: unknown): DeltaBriefRequest | null {
  if (typeof body !== "object" || body === null) return null;
  const s = (body as DeltaBriefRequest).snapshot as DeltaSnapshot | undefined;
...
export async function POST(req: NextRequest) {
  ...
  let parsed: DeltaBriefRequest | null = null;
  try {
  ...
  const key = deltaBriefFingerprint(parsed);
  const cached = getCachedDeltaBrief(key);
  if (cached) {
    return NextResponse.json(
      { ...cached, cached: true },
  ...
  try {
    const { brief, costUSD } = await generateDeltaBrief(parsed);
    const payload = { brief, generatedAt: new Date().toISOString(), cached: false, costUSD };
    setCachedDeltaBrief(key, payload);
    return NextResponse.json(payload, { headers: { "Cache-Control": "no-store" } });
  } catch (err) {
    const { status, error } = deltaBriefErrorResponse(err);
