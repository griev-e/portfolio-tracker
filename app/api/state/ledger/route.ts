...
/**
 * PUT /api/state/ledger — compare-and-swap upsert of the signed-in user's
 * theta ledger blob. The client sends the revision it last hydrated/wrote in
...
export async function PUT(req: Request) {
  ...
  const result = await putLedger(guard.userId, body.value, baseRev);
  if (!result.ok) {
